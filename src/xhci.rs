...
impl CapabilityRegisters {
    ...
    fn num_scratchpad_bufs(&self) -> usize {
        (extract_bits(self.hcsparams2.read(), 21, 5) << 5
            | extract_bits(self.hccparams2.read(), 27, 5)) as usize
    }
...
// and supports up to 255 USB devices of hubs
// [xhci_1_2] p.59
#[repr(C, align(64))]
...
impl DeviceContext {
    ...
    fn set_last_valid_dci(&mut self, dci: usize) -> Result<()> {
        // - 6.2.2:
        // ...the index (dci) of the last valud Endpoint Context
...
impl PortSc {
    fn new(bar: &BarMem64, cap_regs: &CapabilityRegisters) -> Self {
        ...
        for port in 1..=num_ports {
            // SAFETY: Tihs is safe since the result of ptr calculation
...
impl Doorbell {
    ...
    pub fn notify(&self, target: u8, task: u16) {
        ...
        // SAFETY: Tihs is safe as long as the ptr is valid
        unsafe {
...
impl SetupStageTrb {
    ...
    pub fn new(request_type: u8, request: u8, value: u16, index: u16, length: u16) -> Self {
        // Table 4-7: USB SETUP Data to Stage TRB and Status Stage TRB
        // mapping
...
// Status stage direction will be opposite to the data.
