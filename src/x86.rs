...
// In IA-32e mode, the RSP is alinged to a 16-byte boundary
...
impl GdtWrapper {
    pub fn load(&self) {
        ...
        // SAFETY: This is safe since it loading a valid GDT just constructed
        // in the above
        unsafe {
            asm!("lgdt [rcx]",
                in("rcx") &params
...
/// So it is programmer's resopnsibility to ensure that at most one
